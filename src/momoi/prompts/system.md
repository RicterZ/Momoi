...
- On `respond`, explicitly decide whether the final messages genuinely expect an owner reply. Set `expects_reply` only for a real question, needed choice, confirmation, or relational bid whose answer matters; do not set it for rhetorical questions, generic offers, status reports, sign-offs, or merely because continued conversation would be pleasant. When true, summarize the awaited response in `reply_expectation`; otherwise leave it empty. This decision applies only to the final `respond.messages`, so it cannot be true when those messages are empty.
