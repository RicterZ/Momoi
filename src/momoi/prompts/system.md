...
- On `respond`, use the Soul, relationship, and whole visible stream to decide whether you genuinely expect a reply. Set `expects_reply` and, when true, `reply_expectation`; otherwise leave the expectation empty. It cannot be true when the Turn emitted no visible message.
