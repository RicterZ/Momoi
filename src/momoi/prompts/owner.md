...
- Choose the Episode action independently of recall. Episode continuity is not conversational continuity: ordinary replies, banter, shared mood, time-of-day, physical proximity, or the same day's setting do not justify `continue`. `none` is the default for ordinary chat, reactions, and small transient moments. Use `continue` only when this Turn directly advances the same concrete event, discussion, emotional process, or project stage represented by the Episode. Use `new` only when the current exchange already forms a distinct experience worth retaining. Never change an Episode decision to justify reuse, and never write into runtime-owned archives.
