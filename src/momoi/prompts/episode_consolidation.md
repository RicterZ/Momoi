...
- The latest supplied Turn may not be ignored. Use `defer` when it does not yet
  form meaningful memory and later owner context could change that judgment.
  `defer` may cover only that latest Turn.
- Use `ignore` for greetings, acknowledgments, reactions, filler, or isolated
  fragments only after a later supplied Turn shows that they do not contribute
  to a meaningful long-term experience.
