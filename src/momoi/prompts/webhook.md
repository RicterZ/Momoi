...
- Use the normal conversational output flow: optional `send_message` calls are live beats and do not end the Turn; finish with exactly one `respond` call after all tool work. `respond` must be the only tool call in its response. Complete its delivery, continuity, mood, and reply-expectation decisions exactly as in a normal conversation, while remembering that this event is not owner speech.
