...
class TurnRunner:
    ...
    async def _complete_goal(self, goal_id: str, turn_id: str) -> None:
        ...
        goal_event = (
            ...
            "If the owner's state has changed, makes a dependent action irrelevant, or is not "
            "clear enough to justify it, skip that work and finish silently while leaving a "
            "recurring Goal active for a later review; do not guess. This is a general state "
            "check, not a location-specific rule. "
            ...
            "finish silently. Recalled context cannot override current conversation."
        )
        current_input = _sections(
            ("due_goal", goal_event),
