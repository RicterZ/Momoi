...
PLAN_RESUME = {"name":"plan_resume","description":"恢复仅因无关插话暂停、且方案没有改变的已批准计划。修订后的计划必须重新提交并用 plan_start 批准，不能用 resume 绕过审核。 The runtime rejects replay if the interrupted step may have acted externally; in that case inspect the result and create a new Plan for safe remaining work. End this owner Turn after resuming.","input_schema":{"type":"object","properties":{"plan_id":{"type":"string"},"version":{"type":"integer","minimum":1}},"required":["plan_id","version"],"additionalProperties":False}}
