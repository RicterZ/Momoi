...
from ..errors import http_category, error_category
...
logger = logging.getLogger("momoi.integrations.adapters.fish")
...
class FishAudioTTSProvider(TTSProvider):
        ...
        *,
        ...
        for name, value in {
            ...
            if (
                ...
                raise ValueError(
                    f"Fish TTS {name} must be a nonempty single-line string"
                )
        ...
        if self.model not in FISH_MODELS:
            raise ValueError(
                "Fish TTS model must be one of: " + ", ".join(sorted(FISH_MODELS))
            )
        ...
        if (
            ...
            raise ValueError(
                "Fish TTS base_url must be an absolute HTTP URL without credentials, query or fragment"
            )
    ...
    async def synthesize(self, text: str) -> AudioOutput:
        ...
        def failed(error, attempt, attempts, again):
            log_event(
                logger,
                logging.WARNING,
                "tts_request_failed",
        ...
        try:
            return await retry_call(
                ...
                delays=TTS_RETRY_DELAYS,
                retryable=lambda error: isinstance(error, TTSError),
                on_error=failed,
            )
        except TTSError as error:
            raise TTSError(
                f"{error} (failed after {len(TTS_RETRY_DELAYS) + 1} attempts)",
    ...
    def _error_detail(self, detail: str, text: str) -> str:
        for private in (self.api_key, self.reference_id, text):
            for value in (
                private,
                json.dumps(private, ensure_ascii=True)[1:-1],
    ...
    async def _synthesize_once(self, text: str) -> AudioOutput:
        try:
            ...
            async with self.transport.session(
                ...
                async with session.post(
                    ...
                    if response.status != 200:
                        body = (await response.content.read(8192)).decode(
                            "utf-8", errors="replace"
                        )
                        detail = self._error_detail(body, text)
                        raise TTSError(
                            f"Fish TTS returned HTTP {response.status}: {detail}",
                    ...
                    if (
                        ...
                        and response.content_length > self.max_audio_bytes
                    ):
            ...
            raise TTSError(
                f"Fish TTS request failed: {self._error_detail(detail, text)}",
                category=error_category(error),
                service="fish",
                operation="synthesize",
                retryable=True,
