...
`recent_episode_hours` 设置近期 Episode 基线的活跃时间窗口；`summary_results` 设置查询召回的 Episode 数量上限；`summary_tokens` 设置合并后 Episode 摘要共享的 token 预算。
