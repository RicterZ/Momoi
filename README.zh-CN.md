...
- 支持图像的模型可以理解对话中的图片。
- 表情包和反应图片可以只用来加强语气或自然收尾，不会被强行解读成新话题。
- 处理长任务时，可以及时发送有价值的进度。
...
近期对话会跟随 Momoi 进入主人消息、目标、Webhook、回复等待和心跳。确认送达的记录让之后的对话知道她真正说过什么；送达不确定或失败的内容不会被当成双方已经共有的事实。
...
| 回复跟进 | Momoi 确实在等待一个回答 | 短暂等待，必要时发送一条贴合上下文的小跟进，但不会借机开启新话题 |
...
- 发送文本、渠道原生媒体和受管理的图片反应
- 跨对话保持情绪和活动状态
- 每日复盘主人、世界、自身与执行经验，并形成有来源的长期学习
- 通过受限心跳自主工作并主动说话
- 使用 `/stop` 停止正在进行的工作
- 使用 `/heartbeat` 手动触发一次心跳
- 在外部操作结果不确定时安全恢复
